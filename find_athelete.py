...
def request_data():
    ...
    """
    print("Поиск атлета, ближайшего к Вам по различным параметрам.")
...
def nearest_by_height(user, session):
    ...
    """
    athletes_list = session.query(Athelete).filter(
        Athelete.height != None).all()
    atlhete_id_height = {
        athlete.id: athlete.height for athlete in athletes_list}
...
def main():
    ...
    if not user:
        ...
        print(
            "Ближайший по дате рождения атлет: {}, его дата рождения: {}".format(
                bd_athlete, bd)
        )
        print(
            "Ближайший по росту атлет: {}, его рост: {}".format(
                height_athlete, height)
        )
